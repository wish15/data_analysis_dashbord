...
class Text:
  def __init__(self,no_of_topics=None, scan=False):
    self.file_path=None
    self.text=None
    self.url=None
    self.no_of_topics=no_of_topics
    self.scan=scan
    stopword_=pd.read_pickle('C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\dictionary\\text_stopwords.pkl')
    positive_dict=pd.read_pickle('C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\dictionary\\text_pos.pkl')
    negative_dict=pd.read_pickle('C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\dictionary\\text_neg.pkl')
    constraints=pd.read_pickle('C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\dictionary\\text_constraints.pkl')
    uncertain=pd.read_pickle('C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\dictionary\\text_uncertain.pkl')
  ...
  def __pdf_to_image(self,file_path):
    ...
    pages = convert_from_path(file_path, poppler_path="C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\poppler-0.68.0\\bin")
    return pages
  #im-->PIL image obtained from pdf
  def __image_preprocessing(self,im):
  
    #save image as jpeg file in this notebook
    im.save('C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\out_img.jpg', 'JPEG')
    '''
    ...
    '''
    img=cv2.imread('C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\out_img.jpg', cv2.COLOR_BGR2RGB )
  ...
  def __file_path_ext(self,file_path, url):
    if self.scan==True:
      ...
      if(url.startswith("http") or url.startswith("www.")) and (url.endswith(".pdf") or url.endswith(".txt") or url.endswith(".docx") or url.endswith(".odt") or url.endswith(".doc")):
        s=url.split('/')
        print(s)
        urlretrieve(url,"C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\file"+str(s[-1]))
        text=textract.process("C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\file"+str(s[-1]))
  ...
  def __preprocess_text_readability(self,text):
    ...
    for s in word:
      ...
      #returns the preprocessed tokenize text and total no of sentence
  ...
  def topic_modelling(self,file_path=None, url=None, text=None):
    ...
    pyLDAvis.save_html(vis,"C:\\Users\\Win- 10\\Desktop\\Final_project_Server\\abc.html")
